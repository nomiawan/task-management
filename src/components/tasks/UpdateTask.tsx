...
const UpdateTask: React.FC<ModalProps> = ({
  isModalOpen,
  handleCancel,
  tId,
}) => {
  ...
  useEffect(() => {
    setName(tId?.name);
    setDescription(tId?.description);
    setDate(moment(tId?.due_date).format("YYYY-MM-DD"));
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const res = await updateProjectTaskAction(id, tId?.id, payload);
